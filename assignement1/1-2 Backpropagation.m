...
%%
load('Rs_bayes2.mat');
algs = {'traingd', 'traingda', 'traincgf', 'traincgp', 'trainbfg', 'trainlm', 'trainbr 15', 'trainbr 500'};
figure;
% subplot(2,1,1); plot(2:3:100, Rs_bayes_epochs, 'linewidth', 2);
Rs_bayes_noise = [Rs_bayes_noise, Rs_bayes_500];
% subplot(2,1,2); 
plot(0:0.02:0.9,fliplr(Rs_bayes_noise),'linewidth',2); xlabel('noise rate'); ylabel('Correlation coefficient'); legend(fliplr(algs)); title('Bayesian learning with noise'); ylim([0,1]);
...
load('personal_regression_run_final_models2.mat');
structures = {'[30 30]','[50 50]','[80 80]','[80, 50]', '[50,20]', '[30 30 30]', '[50 50 50]', '[80 50 20]', '[50 20 5]'};
bar(res_valmses); xticklabels(structures); ylabel('Validation set MSE');
text(0.7:1:9, res_valmses+4e-6, num2str(res_epochs))
